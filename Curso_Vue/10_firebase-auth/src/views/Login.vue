<template>
    <div class="container">
        ...
        <form @submit.prevent="ingresarUsuario({email: $v.email.$model, pass: $v.pass.$model})">     <!-- como el nombre de la propiedad es igual a la variable me ahorro escribir {email: email, pass: pass} -->
            ...
            <small class="text-danger d-block my-3" v-if="$v.email.$invalid">Ingrese un email valido</small>
            
            <input type="password" class="form-control my-3" v-model="$v.pass.$model" placeholder="Ingresa una contraseña">
            <small class="text-danger d-block my-3" v-if="!$v.pass.required">Campo requerido</small>
            <small class="text-danger d-block my-3" v-if="$v.pass.minLength">Minimo 6 caracteres</small>
